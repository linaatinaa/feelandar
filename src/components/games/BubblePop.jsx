...
const BUBBLE_COLORS = ['var(--mood-tired)', 'var(--mood-sad)', 'var(--mood-angry)', 'var(--mood-happy)'];
...
export default function BubblePop({ onBack }) {
  ...
  const missBubble = useCallback((id) => {
  ...
  const popBubble = useCallback((id) => {
    const timeoutId = missTimeoutsRef.current.get(id);
    if (timeoutId) {
  ...
  useEffect(() => {
    ...
    spawnIntervalRef.current = setInterval(() => {
      ...
      const timeoutId = setTimeout(() => missBubble(id), duration * 1000);
      ...
      <div className="rounded-3xl p-4 shadow-soft bg-surface border border-border">
        ...
        {status === 'playing' ? (
          <div
            ...
            style={{ height: CONTAINER_HEIGHT, background: 'var(--color-surface-alt)' }}
          >
            <AnimatePresence>
              {bubbles.map((b) => (
                <motion.button
                  ...
                  initial={{ y: 0, opacity: 0.85, scale: 1 }}
